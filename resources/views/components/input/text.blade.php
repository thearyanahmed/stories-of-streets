...
<div class="flex {{ $attributes['fullsizeinput'] ? 'w-full' :'' }} rounded-md shadow-sm">
    ...
    <input {{ $attributes->merge(['class' => 'rounded-md flex-1 form-input border-cool-gray-300 block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5' . ($leadingAddOn ? ' rounded-none rounded-r-md' : '')]) }}/>
</div>
